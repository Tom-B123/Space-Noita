...
int falling_cell_priority(struct Pos pos, int width, float gravity_angle, int step, struct Bound *bounds, __global const short *src_pos) {
    ...
    // Get the movement priority of a cell
    for (int i = 0; i < 8; i++) {
...
struct Pos update_particle(struct Pos pos, int width, int height, float gravity_angle, int n, int step, struct Bound *bounds, __global const short *src_pos) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (is_empty(pos,width,height,src_pos) && falling_cell_priority(pos,width,gravity_angle,step,bounds,src_pos) >= i) {
            if (pos.x >= 0 && pos.x <= width-1 && pos.y >= 0 && pos.y <= height-1) return pos;
        }
...
struct Pos update_powder(struct Pos pos, int width, int height, float gravity_angle, int step, struct Bound *bounds, __global const short *src_pos) {
    return update_particle(pos,width,height,gravity_angle,3,step,bounds,src_pos);
}
...
__kernel void sampleKernel(__global const short *src_pos, __global short *dst_pos, __global const float *src_gravity_angle, __global const int *src_world_dims, __global int *src_step) {
    ...
    // Materials should have a range, eg. 2 in rgb. These colours may correspond to display colours, e.g. metal can be white / red etc.
    struct Bound bounds[] = {
        {29,31,29,31, 0, 2, 0}, // Sand  (ID 0)
        { 0, 2, 0, 2,29,31, 1}, // Water (ID 1)
        {29,31,29,31,29,31, 2}, // Steam (ID 2)
    };
    ...
    if (tag.is_gas) { pos = update_gas(pos,width,height,gravity_angle,step,&bounds,src_pos); }
